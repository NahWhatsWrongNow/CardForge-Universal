...
export function chooseAction(state, profile) {
  ...
  if (strategy === 'aggro' && enemyAttacker && Math.random() < 0.75) {
    return { type: 'attack-hero', attackerId: enemyAttacker.id, trace: 'aggro:push-face' };
  }
  ...
  if (enemyAttacker && Math.random() < (weights.attackFace ?? 0.2)) {
    return { type: 'attack-hero', attackerId: enemyAttacker.id, trace: `weighted:attackFace:${strategy}` };
  }
