...
const state = {
  ...
  deckMode: 'planning', deckSearch: '', selectedDeckId: null, selectedAiId: null, bossMode: false, aiThinking: false, aiThinkingLine: '', lastAiTrace: 'not-run', selectedPlayerSlot: null, selectedAttackerId: null, logHidden: false, usedAttacks: {}, enemyUsedAttacks: {}, currentTurn: 'player',
  playerMinions: [],
  enemyMinions: [
...
function renderPanels() {
  ...
  document.querySelector('#mana').textContent = state.mana;
...
function cleanupDead() {
  ...
  if (state.enemyHealth <= 0) {
    const line = currentAiProfile()?.personality?.victoryLine || `You found ${currentAiProfile()?.personality?.dimensionTag ?? '#Unknown'}.`;
    log(line);
  }
}
